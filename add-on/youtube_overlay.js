class YouTubeOverlay {
    ...
    constructor() {
        ...
        chrome.runtime.onMessage.addListener((message) => {
            switch (message.type) {
                case "overlay":
                    if (this.style) {
                        this.showMessage(message, this.duration);
                    }
                    break;
                case "update_overlay":
                    this._loadOptions();
        ...
        this.overlayText.addEventListener("mousedown", (event) => {
            this._startDragging(event.x, event.y);
        });
    ...
    _startDragging(x, y) {
        ...
        if (this.overlay.style.bottom != "") {
            ...
            this.updateOverlayPosition(0, this.sliderMargin / this.player.clientHeight);
        }
    ...
    /**
     * Drags overlay
     * @param {Number} x Mouse postion x
     * @param {Number} y Mouse position y
     */
    _drag(x, y) {
        // Distance of mouse
        ...
        this.updateOverlayPosition(dX / this.player.clientWidth, dY / this.player.clientHeight);
    }
    ...
    _stopDragging(x, y) {
        ...
        this.pos = this.updateOverlayPosition(dX / this.player.clientWidth, dY / this.player.clientHeight);
    ...
    /**
     * Inits player by adding overlay
     */
    _initPlayer() {
    ...
    /**
     * Updates position of overlay element
     * @param {Number} dX (Optional) X direction
     * @param {Number} dY (Optional) Y direction
     */
    updateOverlayPosition(dX=null, dY=null) {
        let pos = this.pos;
    ...
    /**
     * Updates message when new player size
     */
    _updateSize() {
        if (this.player !== null && this.player.clientHeight != 0 && this.player.clientWidth != 0) {
            if (this.videoWidth != this.player.clientWidth || this.videoHeight != this.player.clientHeight) {
                // Size video
                this.videoWidth = this.player.clientWidth;
                this.videoHeight = this.player.clientHeight;
                ...
                const element = this.overlayText.firstElementChild;
                if (element !== null) {
                    element.style.fontSize = `${this.fontSize}px`;
                }

                // Relative max size of margin [0..1]
                const marginX = 1 - this.overlayWidth / this.player.clientWidth;
                const marginY = 1 - this.fontSize / this.player.clientHeight;

                // Add margin proportionally to pos
                const dMarginX = marginX - this.pos.left - this.pos.right;
                const dMarginY = marginY - this.pos.top - this.pos.bottom;
                const leftRight = this.pos.left + this.pos.right + 1e-3;
                const topBottom = this.pos.top + this.pos.bottom + 1e-3;

                this.pos.left = Math.max(this.pos.left + dMarginX * (this.pos.left / leftRight), 0);
                this.pos.right = Math.max(this.pos.right + dMarginX * (this.pos.right / leftRight), 0);
                this.pos.top = Math.max(this.pos.top + dMarginY * (this.pos.top / topBottom), 0);
                this.pos.bottom = Math.max(this.pos.bottom + dMarginY * (this.pos.bottom / topBottom), 0);

                this.pos = this.updateOverlayPosition();
            }
        }
    }
     ...
     * @param {Object} style JSON object with HTML styles
     */
    setStyle(style) {
        this.style = style;
        ...
        this._updateSize();
        this._applyStyle(element);
    }

    /**
     * Applies style for overlay text and background
     * @param {HTMLElement} element HTMLElement for text
     */
    _applyStyle(element) {
     ...
     * @param {Object} message JSON with YouTube live-chat author, message, and rawMessage
     * @param {String} align Alignment of message [null, "left" | "center"], null represents "left"
     */
    _parseOverlayMessage(message) {
        ...
        for (let obj of message.rawMessage) {
            ...
            if (obj.src) {
                ...
                if (!this.translatorTags) {
                    string = string.replace(/\[(TL)?\/?([A-Za-z]{2}|.語|.訳)\]:?/g, "");
                }
     ...
     * @param {Object} message JSON with YouTube live-chat author, message, and rawMessage
     * @param {Number} duration Duration in ms message is shown, null shows the message until showing a new message
     */
    showMessage(message, duration=null) {
        if (!this.disabled) {
            const element = this._parseOverlayMessage(message);
     ...
     * @param {Boolean} disable_overlay Disables overlay, else enables overlay
     */
